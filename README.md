...
仓库根目录的 `SKILL.md` 是本项目的**领域专用** AI 编辑层，而不是一套通用 Agent 工作流。它只保留因果推理教材中特别容易发生的概念错位与核验要求，例如区分因果问题、目标量、识别、估计和解释，保持潜在结果与结构因果模型的语义边界，防止把因果发现稳定性、预测性能或算法输出误写成因果正确性。

通用的范围控制、工具使用、证据诚实和完成报告可以由上层 Agent、Harness 或独立的轻量通用 Skill 提供，不在本仓库重复定义。领域层的深化优先表现为补足关键区分、失败诊断、推理桥梁和适用边界，而不是增加篇幅、术语数量或引用密度。最终仍以 `make check` 作为工程验收标准。
...
- AI 辅助编辑遵循 `SKILL.md` 的领域约束，但不把其中列举的问题当作穷尽性 checklist。
