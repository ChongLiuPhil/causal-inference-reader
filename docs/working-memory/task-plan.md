...
- WM-T006 — current AHICP / PPF / Vault / Starter upgrade — IN REVIEW

## NEXT ACTIONS

1. 人类在 GitHub Settings → Pages 中确认 Source = “Deploy from a branch”，Branch = `gh-pages`，folder = `/(root)`。
2. 公网可访问后完成 WM-T004 逐页验收。
3. 验收后转入正常内容维护，并按新的作者审核规则继续扩写或整理。
...
- current Stack adoption 在 PR 合并前保持 validation-pending；provider binding 不因 Stack upgrade 自动解决。
